...
public class TaskByTriggerWindow extends BasicWindow {
    private ActionListBox tasks;
    private Button exit;

    public TaskByTriggerWindow() {
        super("Run Tasks By Trigger");
        ...
        for (Task.Trigger t : Task.Trigger.values()) {
            ...
            tasks.addItem(num + t.name(),
                    () -> getTextGUI().addWindowAndWait(new LoggingExecutingDialog(t.name(), () -> {
                        for (Class<? extends AbstractTask> c : TaskManager.getTasksForTrigger(t)) {
                            TaskManager.runTaskByClass(c);
                        }
                    })));
        }
